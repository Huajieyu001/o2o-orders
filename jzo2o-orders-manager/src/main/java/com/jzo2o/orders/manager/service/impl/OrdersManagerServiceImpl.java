...
//        orderStateMachine.changeStatus(orders.getUserId(), orders.getId().toString(), OrderStatusChangeEventEnum.EVALUATE, orderSnapshotDTO);
    ...
    public void cancel(OrderCancelDTO orderCancelDTO) {
        ...
        if (OrderStatusEnum.NO_PAY.getStatus().intValue() == ordersStatus) { //订单状态为待支付
            owner.cancelByNoPay(orderCancelDTO);
        } else if (OrderStatusEnum.DISPATCHING.getStatus().intValue() == ordersStatus) { //订单状态为待服务
    ...
    private Orders canalIfPayOvertime(Orders orders){
        //创建订单未支付15分钟后自动取消
        if(orders.getOrdersStatus().intValue() == OrderStatusEnum.NO_PAY.getStatus() && orders.getCreateTime().plusMinutes(15).isBefore(LocalDateTime.now())){
